 ...
 *
 * An object can be freely duplicated, immutably combined with another JS object, and
 * can be mutated by setting arbitrary values on it. All of this just like in JS.
 *
...
object StObject {
  ...
  @inline
  def apply(): StObject =
    js.Dynamic.literal().asInstanceOf[StObject]
  ...
  @inline
  implicit class StObjectCast2Ops[Self[_, _] <: StObject, T1, T2](val x: Self[T1, T2]) extends AnyVal {
  ...
  @inline
  implicit class StObjectCast3Ops[Self[_, _, _] <: StObject, T1, T2, T3](val x: Self[T1, T2, T3]) extends AnyVal {
