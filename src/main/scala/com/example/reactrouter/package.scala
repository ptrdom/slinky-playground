...
package object reactrouter {
  ...
  def withRouter[T <: BaseComponentWrapper](
      bc: T
  )(implicit ev: bc.Props <:< RouteProps, propsReader: Reader[bc.Props], ctag: ConstructorTag[bc.Def]): ReactElement = {
    val bb = ReactComponentClass.wrapperToClass(bc).asInstanceOf[ReactComponentClass[RouteProps]]
